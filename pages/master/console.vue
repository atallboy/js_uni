		...
		<view class="col-menu-container" style="margin-top: 20rpx;">
			<view class="col-menu-wrap">
				<view class="col-menu-list">
					...
					<view class="col-menu-item" @click="toWhere('/pages/balance/record?record_cate=master_fee')">
						<view class="col-menu-icon">
		...
		<view class="col-menu-container" style="">
			<view class="col-menu-wrap">
				...
				<view class="col-menu-list">
					...
					<view class="col-menu-item" @click="toWhere('order?status=101')">
						<view class="col-menu-icon">
							<text>{{master.calc.order_all}}</text>
						</view>
						<view class="col-menu-name">全部</view>
					</view>
					<view class="col-menu-item" @click="toWhere('order?status=0')">
						<view class="col-menu-icon">
							<text>{{master.calc.order_wait_pay}}</text>
						</view>
						<view class="col-menu-name">待支付</view>
					</view>
...
<!-- 					<view class="col-menu-item" @click="toWhere('order?status=2')">
					...
					<view class="col-menu-item" @click="toWhere('order?status=1')">
						<view class="col-menu-icon">
							<text>{{master.calc.wait_taking}}</text>
						</view>
						<view class="col-menu-name">待接单</view>
					</view>		
					<view class="col-menu-item" @click="toWhere('order?status=20')">
						<view class="col-menu-icon">
							<text>{{master.calc.wait_taking}}</text>
						</view>
						<view class="col-menu-name">已接单</view>
					</view>									
					<view class="col-menu-item" @click="toWhere('order?status=21')">
						<view class="col-menu-icon">
							<text>{{master.calc.master_departure}}</text>
						</view>
						<view class="col-menu-name">已出发</view>
					</view>
					<view class="col-menu-item" @click="toWhere('order?status=22')">
						<view class="col-menu-icon">
							<text>{{master.calc.master_arrival}}</text>
						</view>
						<view class="col-menu-name">已到达</view>
					</view>
					<view class="col-menu-item" @click="toWhere('order?status=23')">
						<view class="col-menu-icon">
							<text>{{master.calc.master_start}}</text>
						</view>
						<view class="col-menu-name">已开始</view>
					</view>
					<view class="col-menu-item" @click="toWhere('order?status=24')">
						<view class="col-menu-icon">
							<text>{{master.calc.master_complete}}</text>
						</view>
						<view class="col-menu-name">服务完成</view>
					</view>																				
					<view class="col-menu-item" @click="toWhere('order?status=3')">
						<view class="col-menu-icon">
							<text>{{master.calc.order_complete}}</text>
						</view>
						<view class="col-menu-name">订单完成</view>
					</view>
<!-- 					<view class="col-menu-item" @click="toWhere('order?status=5')">
					...
					<view class="col-menu-item" @click="toWhere('order?status=11')">
						<view class="col-menu-icon">
							<text>{{master.calc.order_apply_refund}}</text>
						</view>
						<view class="col-menu-name">申请退款</view>
					</view>
					<view class="col-menu-item" @click="toWhere('order?status=12')">
						<view class="col-menu-icon">
							<text>{{master.calc.order_refund}}</text>
						</view>
						<view class="col-menu-name">已退款</view>
					</view>										
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			startadress() {
				...
				uni.getLocation({
					...
					success: (res) => {
						...
						// this.jindu = longitude; // 将经度存储到组件的属性中
						// this.weidu = latitude; // 将纬度存储到组件的属性中
						// console.log('经度', longitude);
						// console.log('纬度', latitude);
						// this.reverseGeocoding(longitude, latitude); // 调用逆地理编码函数
					...
					complete: (complete) => {
						console.log('获取定位成功与否都返回', complete);
					}
				});
			},
			...
			reverseGeocoding(longitude, latitude) {
				...
				const apiKey = '4cd4bcf64f841a86adbb36d879ccd066';
				...
				fetch(apiUrl)
					...
					.then(data => {
						if (data.status === '1' && data.regeocode) {
							...
							console.log('打印转码后的地址', this.locationName);
