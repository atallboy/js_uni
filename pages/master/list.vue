<template>
	<view>
		<view class="search-container">
			<view class="search-picker" @click="selCity">
				...
				<view class="list-item" v-for="(item,k) in list">
					...
					<view class="brief">
						...
						<view class="brief-data">
							...
							<image src="../../static/images/collect-ed.png" @click="collect(item.id)" v-if="item.is_collected==1"></image>
							<image src="../../static/images/collect.png" @click="collect(item.id)" v-else></image>
...
<!-- 				<view>
					<uni-swiper-dot :info="master.picArr" field="content">
						<swiper class="swiper-box"  :autoplay=true style="height: 260px;">
							<swiper-item v-for="(item,index) in master.picArr" :key="index"  @click="previewPic(item,master.picArr)"> 
								<view class="swiper-item" style="width: 100%;height: 250px;" >
									<image :src="item" style="width: 100%;height: 100%;border-radius: 1px;object-fit: cover;"></image>
								</view>
							</swiper-item>
						</swiper>
					</uni-swiper-dot>	
...
<view style="padding-bottom: 20px;">
	<scroll-view style="height: 300px;" scroll-y="true">
		<view v-for="(item, index) in master.picArr" :key="index" @click="previewPic(item, master.picArr)">
			<view style="width: 100%; padding-bottom: 10px;text-align: center;">
				<image :src="item" style="width: 95vw; border-radius: 1px; height: 95vw;margin: 0 auto; "></image>
			</view>
		</view>
	</scroll-view>
</view>
...
<script>
	import {
		settingInfo,getUserInfo,sendSms,verifySms,
		masterList,
		itemList,
		masterCollect,
		evaList,
	...
	export default {
		...
		onShow() {
			...
			if(cityData){
				...
				this.cityData = {name:'未知城市',id:false}
			}
		...
		methods: {
			...
			selItem(e, k) {
				...
				// })
				uni.navigateTo({
					url: '/pages/order/submit?item_id='+e.id+'&master_id='+this.master.id
				})
			},
			changeCity(e) {
			...
			async getMasterList() {
				this.condition.city = this.cityData.name
