...
<script>
	import { orderList } from '@/utils/api.js'
	export default {
		data() {
			return {
				scrollLeft:0,
				cate:[
					// {id:0,active:'tab-item-active',name:'全部订单'},
					{id:1,active:'',name:'待开始'},
					{id:2,active:'',name:'进行中'},
					{id:3,active:'',name:'已完成'},
					// {id:5,active:'',name:'待反馈'},
					{id:5,active:'',name:'已取消'}
				],
				status:1,
				list:[],
			}
		...
		onLoad(e) {
			if(e.status){
				...
				for(let i=0;i<this.cate.length;i++){
				  if(e.status==this.cate[i].id){this.cate[i].active='tab-item-active';k=i }
				  else{this.cate[i].active='';}
				}
		...
		methods: {
			detail(e){
				uni.navigateTo({
					url:'/pages/order/detail?user_type=agent&id='+e
				})
			},
			async orderList() {
				const res = await orderList({status:this.status,user:'agent'})
...
.order-wrap{}
.order-item{background-color: #fff;margin-bottom: 10px;border-radius: 6px;display: flex;flex-direction: column;}
.order-item-header{display: flex;color: var(--mainColor);background: rgb(242, 255, 248);padding: 5px 10px;}
.order-item-status{display: flex;flex: 1;}
.order-item-master{display: flex;flex: 1;justify-content: flex-end;}
.order-item-name{font-size: 16px;font-weight: 600;padding: 5px 10px;}
.order-item-info{display: flex;padding: 6px 10px;}
.order-item-time{display: flex;flex: 1;}
.order-item-price{display: flex;flex: 1;justify-content: flex-end;color: rgb(255, 96, 81);font-size: 19px;font-weight: 600;}
.order-item-act{display: flex;justify-content: flex-end;padding: 6px 10px;}
.order-item-btn{width: 66px;text-align: center; font-size: 12px;border: 1px solid #333;border-radius: 30px;padding: 3px 0;}
