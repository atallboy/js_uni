...
<!-- 		<view style="position: fixed;left: 0;right: 0;bottom: 0;z-index: 9999;background-color: #fff;padding: 8px 0;display: flex;">
		...
		<view style="background-color: #fff;padding: 15px 10px;margin-bottom: 10px;">
			<view style="color: #000;font-weight: 700;font-size: 14px;margin-bottom: 13px;">{{data.name}}</view>	
			  <view style="display: flex;align-items: center;color: #555;" @click="openMap">
				  <uni-icons type="location-filled" color="#1890ff" size="20"></uni-icons> {{data.province}}{{data.city}}{{data.district}}{{data.address}}
			  </view>				
		</view>
		
		<view style="padding-bottom: 80px;">
			<view style="text-align: center;color: #000;font-weight: 600;">商家介绍</view>
			<view>
...
<script>
	import {mchDetail} from '@/utils/api.js'
	export default {
		data() {
			return {
				id:0,
		...
		methods: {
			...
			async mchDetail(){
				const res = await mchDetail({id:this.id})
